...
"""
struct MIPSHash{T,S} <: LSHFamily{T} where {T, S <: AbstractArray{T}}
	coeff_A :: S
	coeff_B :: S
	denom :: T
	shift :: Vector{T}
...
function MIPSHash{T}(input_length::Integer, n_hashes::Integer, denom::Real, m::Integer) where {T <: LSH_DTYPES}
...
=#
function MIPSHash_P_LSH(h :: MIPSHash{T}, x :: AbstractArray) where {T <: LSH_DTYPES}
...
MIPSHash_P_LSH(h :: MIPSHash{T}, x :: AbstractArray{<:Real}) where {T <: LSH_DTYPES} =
	MIPSHash_P_LSH(h, T.(x))

MIPSHash_P_LSH(h :: MIPSHash{T}, x :: AbstractArray{T}) where {T <: LSH_DTYPES} =
	invoke(MIPSHash_P_LSH, Tuple{MIPSHash, AbstractArray}, h, x)

function MIPSHash_Q_LSH(h :: MIPSHash{T}, x :: AbstractArray) where {T <: LSH_DTYPES}
	# First, perform a matvec on x and the first array of coefficients.
	# Note: aTx is an n_hashes × n_inputs array
	aTx = h.coeff_A * x
...
MIPSHash_Q_LSH(h :: MIPSHash{T}, x :: AbstractArray{<:Real}) where {T <: LSH_DTYPES} =
	MIPSHash_Q_LSH(h, T.(x))

MIPSHash_Q_LSH(h :: MIPSHash{T}, x :: AbstractArray{T}) where {T <: LSH_DTYPES} =
